...
class ImageLib {
	...
	function create_thumb($source,$type,$width=0,$height=0,$newimage=null){
		$this->clear();
	...
	function check_color($R,$G,$B){
		$max = max(array($R,$G,$B));
		$min = min(array($R,$G,$B));
// edit by porter at 2013-12-16 17:35:21修改图片加载背景颜色
		if(($max-$min)<30&&$max<160){
			//return 'black';
			return '#26282E';
		}elseif (($max-$min)<30&&$min>=100&&$max<=240){
			//return 'gray';
			return '#7F7D78';
		}elseif (($max-$min)<30){
			return 'white';
		...
		//	return 'green';
			return '#565D18';
		}elseif(($R>=$G&&($R-$G)<($G-$B))||($G>=$R&&$G>$B&&($G-$R)<30)){
		//}elseif(($R>=$G&&$G>150&&$B<100)){
		//	return 'yellow';
			return '#CBB56B';
		}elseif ($R>$B&&$B>120&&$G<100){
		//	return 'purple';
			return '#492E5D';
		//}elseif ($R>$B&&$R>$G&&min(array(($R-$G),($R-$B)))>140){
		}elseif ($R>$B&&$R>$G){
		//	return 'red';
			return '#981619 ';
		}else{
	...
	function sample_color($source){
		$this->clear();
		$config['source_image'] = $source;
		$config['master_dim'] = 'auto';
		$config['create_thumb'] = TRUE;
		$config['thumb_marker'] = '_sample';
		$config['maintain_ratio'] = TRUE;
		$config['quality'] = 100;
		$config['width'] = 20;
		$config['height'] = 20;
		...
		$g = ($rgb >> 8) & 0xFF;
		$b = $rgb & 0xFF;
		return $this->check_color($r,$g,$b);
	}

	function crop_square($source,$horw,$type='square',$newimage=null){
		$this->clear();
		$config['master_dim'] = 'auto';
		$config['source_image'] = $source;
		$config['dynamic_output'] = false;
	...
	function crop_image($source,$dest,$x,$y,$w,$h){
		$this->clear();
		$config = array();
		$config['source_image'] = $source;
	...
	function sample_rgb()
	{
		...
		return $this->$protocol('sample_rgb');
	}
	...
	function image_process_gd($action = 'resize')
	{
		...
		if ($this->dynamic_output === FALSE)
		{
			...
			{
				if ($this->source_image != $this->new_image&&$action != 'sample_rgb')
				{
		...
		if($action == 'sample_rgb'){
			$x_axis = $this->orig_width/2 - ($this->width/2);
			$y_axis = $this->orig_height/2 - ($this->height/2);
			$copy($dst_img, $src_img, 0, 0, $x_axis, $y_axis, $this->width, $this->height, $this->width,$this->height);
			$dst_img2 = $create(1, 1);
			$copy($dst_img2, $dst_img, 0, 0, 0, 0, 1, 1, $this->width,$this->height);
			$rgb = imagecolorat($dst_img2, 0, 0);
