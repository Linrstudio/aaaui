...
function showSuccess(a, b) {
    show_message("success", getTip(a), !0, 5E3);
    null != b && (!0 == b ? window.location.reload() : window.location.href = b)
}

function showError(a) {
    "not-login" == $.trim(a) ? openLoginDialog() : null != a ? show_message("error", getTip($.trim(a)), !1, 0) :'' //show_message("error", getTip("error") + ": " + getTip("server-error"), !1, 0)
}
       ...
       // end edit
            ...
            f, function (f) {
                getTpl(d, function () {
                    ...
                    j.elem.addClass(h);
                    "forwarding" == c ? (a("#forwarding_div").attr("data-sid", b),$('.img-thumb img').attr('src',f.data.share.image_path+'_middle.jpg'), a("#forwarding_div").attr("data-uid", 0), w("forwarding_div", null, null), j.show()) : "edit_forwarding" == c ? (j.head(g.attr("data-edit-title")), a("#forwarding_div").attr("data-sid", b), a("#forwarding_div").attr("data-edit",
    ...
    function w(b, c, d) {
        ...
        if ("0" == g) {
            ...
            ajaxget(t.attr("data-albumlist-url"),
                g, function (g) {
                 //默认选中第一个专辑
                    a("." + b).find(".album_select_title").html(g.data[0].album_title);
                    a("." + b).find("#ablum_select_id").val(g.data[0].album_id);
                    var k = a(Mustache.render('{{#data}}<li><a href="javascript:;" data-aid="{{album_id}}" data-action="albumItem" data-params="{{album_id}},{{album_title}},' + b + '">{{album_title}}</a></li>{{/data}}', g));
                    j.prepend(k);
                    f.attr("data-init", 1);
...
//edit by porter at 2013-12-03 10:42:21
    function r(b) {
        var c = a("#" + b),
          //  b = c.find(".category_select_id").val(),
            d = c.find(".album_select_id").val(),
            f = c.attr("data-sid"),
            c = c.attr("data-edit"),
            intro = $('#intro').val(),
            g = t.attr("data-forwarding-url");
        1 == c && (g =
            t.attr("data-editshare-url"));
        ajaxpost(g, {
            'share_id': f,
            'album_id': d,
            'intro' : intro,     
    ...
    function Q(b, c, d, f, g) {
        ...
        ajaxpost(t.attr("data-addcomment-url"), b, function (b) {
            b = a(Mustache.to_html(h.html(), b));                                            
             //如果评论数超过5.,就删掉最后一个
            a("#" + c + "_comments").prepend(b);
            if(a("#" + c + "_comments .comment").length>5) {
                a("#" + c + "_comments .comment:last-child").remove();
            } 
            //edit by porter at 2013-12-19 15:48:32弹出详细页评论修复不能及时更新数据的问题
            a("#" + c + "_comments_a").prepend(b); //详细页面的评论   
            // end edit
            b.fadeIn(2E3);
            // edit by porter at 2013-12-09 16:56:01 add a('#'+c+"_comment").text(Number(a('#'+c+"_comment").text())+1);--评论成功数字加一
            a('#'+c+"_comment").text(Number(a('#'+c+"_comment").text())+1); // 瀑布流评论个数
            $('.the-comment .meta-head').text(Number($('.the-comment .meta-head').text())+1); // 图片详细页评论个数
            "comment_detail_tpl" == h ? a.oFEditor("r_editor").writeEditorContents("") :
                a("#" + c + "_commentbox").val("");
            j && (a("#" + c + "_commentdiv").addClass("hide"), a("#waterfall").masonry("reload"));
    ...
    */
    function commentReply(comment_id,content){
        ...
        ajaxpost(t.attr("data-commentreply-url"), data, function (b) {
            ...
            g ? a("#" + comment_id + "_reply").prepend(b) : (a("#" + comment_id + "_reply").prepend(b),a("#" + comment_id + "_reply .comment:last-child").remove());
            //edit by porter at 2013-12-19 15:48:32弹出详细页评论修复不能及时更新数据的问题
            g ? a("#" + comment_id + "_comments_a_reply").prepend(b) : a("#" + comment_id + "_comments_a_reply").append(b);
            // end edit
            b.fadeIn(2E3);
...
//W(t.attr("data-avatarsave-url"), d.x1, d.y1, f, g, c, h, b, "avatar");
    ...
    a("#body").actionController({
        controller: {
            ...
            openAppraiseDialogClick: function () {
                showDialog("publish_url",
                    function () {
                        a("#website_fetch").css('display','none');
                        a("#create_album").css('display','block');
            ...
            applyStoreOpenClick: function (b,shop_id) {
                ...
                //  先点击即认领,不需要填写信息
//                showDialog("apply_store", function () {
//                    a("#apply_parent_cid").change(function () {
//                        var b = {
//                            parentid: a("#apply_parent_cid").val()
//                        };
//                        ajaxpost(t.attr("data-substorec at-url"), b, function (b) {
//                            b = a(Mustache.render('{{#data}}<option value="{{store_category_id}}">{{store_category_name}}</a></option>{{/data}}', b));
//                            a("#apply_sub_cid").html(b)
//                        }, function (a) {
//                            showError(a.message)
//                        })
//                    });
//                    a("#apply_parent_cid").change()
//                });
                return !1
            },
            ...
            // end by allen
            // edit by porter at 2013-12-20 16:45:39网页采集添加拼图功能
            webPinClick: function(a,url,reference_url){
            ...
            sureWebPinClick: function(){
                var reference_url = $('#reference_url').val();
                    url = $('#img').attr('src'),
                    album_id = $('#ablum_select_id').val(),
                    intro = $('#intro').val();
                var data = {
                        ...
                        "album_id" : album_id,
                        "intro" : intro,
                        "share_type" : "web_fetch"
                };
                var f = a("#data-actions");
                ajaxpost(f.attr("web-pin-url"),data, function (a) {
                    showSuccess(a.message);
                    getStaticDialog().hide(); //隐藏当前窗口
                },function(a){
                    showError(a.message);
                });
            },
            ...
            delCommentClick: function (a, b, c) {
                confirm(getTip("confirm-delete-comment")) && B(b, c);
                return !1
            },
            //喜欢分享
            addLikeClick: function (b, c) {
                ...
                ajaxget(t.attr('data-likeshare-url'),data,function(result){
                    $("#" + c + "-favoriteShare").siblings('.add-comment').after(result.data);
                    $("#" + c + "-favoriteShare").parent().children().last().remove();
                },function(a){
                    showError(a.message);
                });
            },
            ...
            // end edit
            // edit  by porter at 2013-12-19 16:41:45 修改分享信息
            updateShareClick: function (){
                var share_id = a('#share_id').val(),
                    album_id = a('#ablum_select_id').val(),
                    intro = a('#intro').val();
                var data = {
                        "share_id" : share_id,
                        "album_id" : album_id,
                ...
                var f = a("#data-actions");
                ajaxpost(f.attr("update-share-url"),data, function (a) {
                    showSuccess(a.message, t.attr("data-myalbum-url"));
                    getStaticDialog().hide();
                },function(a){
                    showError(a.message);
    ...
    if (null != b && void 0 != b) {
        ...
        b.infinitescroll({  
            ...
            if(current_width < 500){
              $('#waterfall').attr('data-pin-width', '125');
              $('.pin').css({'width':'115px','margin':'0 4px 12px'});
            }
        });
    }
});
